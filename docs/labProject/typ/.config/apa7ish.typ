...
#let conf(
  ...
  // Bibliography
  set bibliography(title: "Referências", style: "american-psychological-association")
  ...
  // Abstract & Keywords
  if abstract != none {
    heading(outlined: false, numbering: none, text(normal-size, weight: "regular", [Resumo]))
    align(center)[
      #block(
        width: 90%,
        [
  ...
  // Author Note
  if not anonymous {
    heading(outlined: false, numbering: none, text(normal-size, weight: "bold", [Nota do Autor]))
    ...
    // Contact Information
    [A correspondência sobre este artigo deve ser endereçada a
